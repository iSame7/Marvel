...
extension MarvelFactory {
    ...
    func charactersStories(limitMarvelNumberOfResources: Int, charachterId: String, completionHandler: (stories: [Stories]) -> Void, failure: (error: ErrorType?) -> Void) {
        ...
        if let urlParams = self.URLParameters() {
            ...
            // call characters API.
            Alamofire.request(Router.CharactersComics(endpoint: KMarvelFactory_CharactersEndpoint + "/" + charachterId + KMarvelFactory_CharactersStoriesEndpoint, limit: String(limitMarvelNumberOfResources) , urlParams: urlParams)).responseJSON { result in
...
//                            print("jsonDict \(jsonDict)")
                            let rootClass:StoriesRootClass = StoriesRootClass(fromDictionary: jsonDict)
                            completionHandler(stories: rootClass.data.results)
                        }
                        else{
                        }
                    }
