...
final class ImageViewController: UIViewController, UIScrollViewDelegate, UIGestureRecognizerDelegate, UIViewControllerTransitioningDelegate {
    ...
    init(imageProvider: ImageProvider, configuration: GalleryConfiguration, imageCount: Int, displacedView: UIView, startIndex: Int,  imageIndex: Int, showDisplacedImage: Bool, fadeInHandler: ImageFadeInHandler?, delegate: ImageViewControllerDelegate?) {
        ...
        blackOverlayView.backgroundColor = UIColor(red: CGFloat(Float(40) / 255), green: CGFloat(Float(44) / 255), blue: CGFloat(Float(48) / 255), alpha: 1)
...
//            let aspectFitSize = aspectFitContentSize(forBoundingSize: rotationAdjustedBounds().size, contentSize: image.size)
            ...
            //customize character cover photo as per mockup.
            //let sizeWidth = image.size.width - 140
            //let sizeHeight = image.size.height - 140
            let sizeWidth = 140.0
            let sizeHeight = 285.0
            let aspectFitSize = aspectFitContentSize(forBoundingSize: rotationAdjustedBounds().size, contentSize: CGSize(width: sizeWidth, height: sizeHeight))
